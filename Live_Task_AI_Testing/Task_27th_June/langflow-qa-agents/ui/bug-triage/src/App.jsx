...
const STORAGE_KEY = 'bug-triage-config'
      ...
      <header style={{ backgroundColor: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)', padding: '0 24px', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 900, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #ef4444, #f97316)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🐛</div>
            <div>
              <h1 style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0, lineHeight: 1.2 }}>Bug Triage Agent</h1>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Auto-evaluate JIRA bug priority & severity</p>
            </div>
          </div>
      ...
      {showSettings && (
        <div style={{ backgroundColor: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-color)', padding: '20px 24px' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div><label>Flow ID</label><input type="text" value={config.flowId} onChange={e => setConfig(p => ({ ...p, flowId: e.target.value }))} placeholder="e.g. bug-triage-abc123" /></div>
          </div>
        </div>
      )}
      ...
      <main style={{ flex: 1, padding: '32px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: 8 }}>Bug Triage Agent</h2>
            <p style={{ color: 'var(--text-secondary)', maxWidth: 500, margin: '0 auto' }}>Enter a JIRA bug key to automatically evaluate priority, severity, and categorization recommendations.</p>
          </div>
          ...
          <div className="card" style={{ padding: 28 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <input type="text" value={jiraKey} onChange={e => setJiraKey(e.target.value)} placeholder="e.g. PROJ-123" onKeyDown={e => e.key === 'Enter' && runFlow()} />
              </div>
              <button onClick={runFlow} disabled={loading || !jiraKey.trim() || !config.flowId} className="btn btn-primary" style={{ alignSelf: 'flex-start', backgroundColor: '#ef4444' }}>
          ...
          {result && (
            <div className="card" style={{ padding: 24, borderLeft: '4px solid var(--success)' }}>
              <h4 style={{ color: 'var(--success)', marginBottom: 16 }}>✅ Triage Report</h4>
