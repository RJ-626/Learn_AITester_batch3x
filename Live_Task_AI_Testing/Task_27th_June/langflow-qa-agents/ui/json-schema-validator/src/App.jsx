...
const STORAGE_KEY = 'json-schema-config'
  ...
  const runFlow = async () => {
    ...
    try {
      ...
      const body = {
        ...
        input_type: 'chat',
        tweaks: {
          'JSON-Schema-Validator': { json_schema: schema.trim(), json_payload: payload.trim() }
        }
      }
      ...
      <header style={{ backgroundColor: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)', padding: '0 24px', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 900, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #06b6d4, #0891b2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>📁</div>
            <div>
              <h1 style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0 }}>JSON Schema Validator</h1>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Validate payloads against JSON schemas</p>
            </div>
          </div>
      ...
      <main style={{ flex: 1, padding: '32px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: 8 }}>JSON Schema Validator</h2>
            <p style={{ color: 'var(--text-secondary)', maxWidth: 500, margin: '0 auto' }}>Validate JSON payloads against a JSON Schema with multi-threaded batch support.</p>
          </div>
          ...
          <div className="card" style={{ padding: 28 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <textarea value={payload} onChange={e => setPayload(e.target.value)} placeholder={`Paste your JSON payload here...\n\nExample:\n{\n  "id": 1,\n  "name": "Test"\n}`} rows={8} style={{ fontFamily: 'SF Mono, Monaco, monospace', fontSize: '0.85rem' }} />
              </div>
              <div>
                <label>JSON Schema</label>
                <textarea value={schema} onChange={e => setSchema(e.target.value)} placeholder={`Paste your JSON Schema here...\n\nExample:\n{\n  "type": "object",\n  "properties": {\n    "id": { "type": "integer" }\n  },\n  "required": ["id"]\n}`} rows={8} style={{ fontFamily: 'SF Mono, Monaco, monospace', fontSize: '0.85rem' }} />
              </div>
              <button onClick={runFlow} disabled={loading || !payload.trim() || !schema.trim() || !config.flowId} className="btn btn-primary" style={{ alignSelf: 'flex-start', backgroundColor: '#06b6d4' }}>
          ...
          {result && (
            <div className="card" style={{ padding: 24, borderLeft: '4px solid var(--success)' }}>
              <h4 style={{ color: 'var(--success)', marginBottom: 16 }}>✅ Validation Report</h4>
