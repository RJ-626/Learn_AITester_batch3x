...
const STORAGE_KEY = 'test-case-gen-config'
...
function App() {
  ...
  const [projectName, setProjectName] = useState(() => localStorage.getItem(`${STORAGE_KEY}-projectName`) || 'MyProject')
  ...
  useEffect(() => { localStorage.setItem(`${STORAGE_KEY}-projectName`, projectName) }, [projectName])
  ...
  const runFlow = async () => {
    ...
    try {
      ...
      const tweaks = { 'Multi-File-Writer': { project_name: projectName } }
      ...
      <header style={{ backgroundColor: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)', padding: '0 24px', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 900, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #10b981, #059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>📝</div>
            <div>
              <h1 style={{ fontSize: '1.1rem', fontWeight: 700, margin: 0 }}>Test Case Generator</h1>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Generate Playwright E2E framework from JIRA or PRD</p>
            </div>
          </div>
      ...
      {showSettings && (
        <div style={{ backgroundColor: 'var(--bg-secondary)', borderBottom: '1px solid var(--border-color)', padding: '20px 24px' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div><label>Flow ID</label><input type="text" value={config.flowId} onChange={e => setConfig(p => ({ ...p, flowId: e.target.value }))} placeholder="e.g. test-case-gen-abc123" /></div>
            <div>
              <label>Model</label>
      ...
      <main style={{ flex: 1, padding: '32px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div style={{ textAlign: 'center', padding: '30px 0' }}>
            ...
            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: 8 }}>Test Case Generator</h2>
            <p style={{ color: 'var(--text-secondary)', maxWidth: 500, margin: '0 auto' }}>Generate test cases + a complete Playwright E2E framework from JIRA tickets or PRD documents.</p>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
                  <button onClick={() => setSource('jira')} className="btn" style={{ flex: 1, backgroundColor: source === 'jira' ? '#10b981' : 'var(--bg-secondary)', color: source === 'jira' ? 'white' : 'var(--text-primary)' }}>JIRA Ticket</button>
                  <button onClick={() => setSource('doc')} className="btn" style={{ flex: 1, backgroundColor: source === 'doc' ? '#10b981' : 'var(--bg-secondary)', color: source === 'doc' ? 'white' : 'var(--text-primary)' }}>PRD / Spec Document</button>
                </div>
              </div>

              {source === 'jira' ? (
                <div><label>JIRA Story / Ticket Key</label><input type="text" value={jiraKey} onChange={e => setJiraKey(e.target.value)} placeholder="e.g. PROJ-789" /></div>
              ) : (
                <div><label>Document Content</label><textarea value={docContent} onChange={e => setDocContent(e.target.value)} placeholder="Paste your PRD, spec, or user story text here..." rows={8} /></div>
              )}

              <div><label>Project Name (for file output)</label><input type="text" value={projectName} onChange={e => setProjectName(e.target.value)} placeholder="MyProject" /></div>

              <button onClick={runFlow} disabled={loading || !config.flowId || (source === 'jira' ? !jiraKey.trim() : !docContent.trim())} className="btn btn-primary" style={{ alignSelf: 'flex-start', backgroundColor: '#10b981' }}>
          ...
          {result && (
            <div className="card" style={{ padding: 24, borderLeft: '4px solid var(--success)' }}>
              <h4 style={{ color: 'var(--success)', marginBottom: 16 }}>✅ Generated Framework</h4>
